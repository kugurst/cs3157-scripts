	...
	class GraderWorker implements Runnable
	{
		...
		public void run()
		{
			...
			while ((student = uniDirs.poll()) != null) {
				...
				File mdbDir = new File(student, "part1");
				boolean goodCommit = check.checkGitCommits(mdbDir);
				if (goodCommit)
				...
				if (goodMake)
					out.println(student.getName() + " mdb-lookup-server: make+");
				else
					err.println(student.getName() + " mdb-lookup-server: make-");
				...
				File hcDir = new File(student, "part2");
				goodMake = check.checkMake(hcDir, "http-client");
				if (goodMake)
					out.println(student.getName() + " http-client: make+");
				else
					err.println(student.getName() + " http-client: make-");
				...
				twechoSuccess[0] =
					check.testCommand(hcDir,
						"http-client www2.warnerbros.com 80 /spacejam/movie/jam.htm", null, 10);
				twechoSuccess[1] =
					check.testCommand(hcDir,
						"http-client www.gnu.org 80 /software/make/manual/make.html", null, 10);
				twechoSuccess[2] =
					check.testCommand(hcDir, "http-client www.cplusplus.com 80 /index.html", null,
						10);
				...
				if (memErr)
					err.println(student.getName() + " http-client: memory error-");
				else
					out.println(student.getName() + " http-client: memory error+");
				if (leakErr)
					err.println(student.getName() + " http-client: leak error-");
				else
					out.println(student.getName() + " http-client: leak error+");
				...
				if (goodHTMLFiles[0] && goodHTMLFiles[1])
					out.println(student.getName() + " http-client: same file+");
				else
					err.println(student.getName() + " http-client: same file-");
				...
				if (cleanWorked)
					out.println(student.getName() + " http-client: make clean+");
				else
					err.println(student.getName() + " http-client: make clean-");
